...
CLAUDE.md and README.md play distinct roles.

**CLAUDE.md** -- Auto-loaded when Claude enters the directory. The repo-root
CLAUDE.md keeps its standard meaning: project-wide instructions, build/test
commands, key conventions, top-level context Claude needs every session. Nested
CLAUDE.md files are optional indexes -- a small table of filenames,
descriptions, and "read when..." triggers -- so navigating deeper does not pull
in prose Claude may never need. When Claude opens `app/web/controller.py`, it
retrieves just the indexes along that path.
...
files, it does not belong here. Claude reads these only when something (a
CLAUDE.md trigger, an explicit prompt) tells it to.

Just-in-time context: indexes and root instructions load automatically but stay
small; detail loads only when relevant.

The technical writer agent enforces token budgets: ~200 tokens for nested
CLAUDE.md indexes, ~500 for README.md, 100 for function docs, 150 for module
docs. If you are exceeding them, you are probably documenting what code already
shows. Function docs include "use when..." triggers so the LLM knows when to
reach for them.
